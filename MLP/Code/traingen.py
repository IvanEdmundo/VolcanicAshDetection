...
training_generator, steps_per_epoch = balanced_batch_generator(train_data, train_label, sampler=RandomUnderSampler(), batch_size=config.batch_size, random_state=42)
history = model.fit_generator(generator=training_generator,steps_per_epoch=steps_per_epoch,callbacks=[WandbCallback()], epochs=nb_epoch,
                    verbose=1, class_weight = class_weighting,validation_data=(test_data, test_label), shuffle=True) 


print(history.history.keys())
print ( 'ok7')
model1 = model.evaluate(test_data, test_label, batch_size=config.batch_size)
print(model)


y_pred1 = model.predict_classes(test_data, batch_size=config.batch_size)
